 ...
 */
static const char* BITCOIN_PID_FILENAME = "gold-bcrd.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/gbcrio/gbcr>";

    return (strprintf(_("Copyright (C) %i GBCR Developers").translated, COPYRIGHT_YEAR) + " ") +
           "\n" +
           strprintf(_("Copyright (C) 2009-%i GBCR Developers").translated, COPYRIGHT_YEAR) + " " + "\n" +
           "\n" +
...
/** Sanity checks
 *  Ensure that Gold BCR is running in a usable environment with all
...
{
    // Make sure only a single Gold BCR process is using the data directory.
...
bool AppInitMain(NodeContext& node)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if goldbcr is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if goldbcr is started while in a temporary directory.\n",
