...
#include <config/goldbcr-config.h> /* for USE_QRCODE */
#endif
...
void ReceiveRequestDialog::update()
{
    ...
    QString uri = GUIUtil::formatGoldBCRURI(info);
    ...
    if(info.amount)
        html += "<b>"+tr("Amount")+"</b>: " + GoldBCRUnits::formatHtmlWithUnit(model->getOptionsModel()->getDisplayUnit(), info.amount) + "<br>";
...
{
    GUIUtil::setClipboard(GUIUtil::formatGoldBCRURI(info));
}
