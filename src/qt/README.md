This directory contains the Gold BCRQT graphical user interface (GUI). It uses the cross-platform framework [Qt](https://www1.qt.io/developers/).
...
Represents the main window of the Gold BCR UI.
...
ClientModel is used by the main application `goldbcrgui` and several models like `peertablemodel`.
...
Used to process BIP21 payment URI requests. Also handles URI based application switching (e.g. when following a goldbcr:... link from a browser).
...
* UI elements like Gold BCRAmountField, which inherit from QWidget.
* `goldbcrstrings.cpp`: automatically generated
* `goldbcrunits.(h/cpp)`: GBCR / mGBCR / etc handling
* `callback.h`
...
4. Enter "goldbcr-qt" as project name, enter src/qt as location
5. Leave the file selection as it is
6. Confirm the "summary page"
7. In the "Projects" tab select "Manage Kits..."
8. Select the default "Desktop" kit and select "Clang (x86 64bit in /usr/bin)" as compiler
9. Select LLDB as debugger (you might need to set the path to your installation)
10. Start debugging with Qt Creator (you might need to the executable to "goldbcr-qt" under "Run", which is where you can also add command line arguments)
