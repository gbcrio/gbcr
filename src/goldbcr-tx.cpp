...
static void SetupGoldBCRTxArgs()
{
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        ...
        std::string strUsage = PACKAGE_NAME " gold-bcr-tx utility version " + FormatFullVersion() + "\n\n" +
            "Usage:  gold-bcr-tx [options] <hex-tx> [commands]  Update hex-encoded goldbcr transaction\n" +
            "or:     gold-bcr-tx [options] -create [commands]   Create hex-encoded goldbcr transaction\n" +
            "\n";
...
static int CommandLineRawTx(int argc, char* argv[])
{
    ...
    try {
        ...
        if (!fCreateBlank) {
            ...
            // param: hex-encoded goldbcr transaction
