...
The data directory is the default location where GBCR files are stored.
...
---------|--------------------
Linux    | `$HOME/.goldbcr/`
macOS    | `$HOME/Library/Application Support/Gold BCR/`
Windows  | `%APPDATA%\Gold BCR\` <sup>[\[1\]](#note1)</sup>

2. The non-default data directory path can be specified by `-datadir` option.

3. All content of the data directory, except for `goldbcr.conf` file, is chain-specific. This means the actual data directory paths for non-mainnet cases differ:
...
`blocks/`          | `blkNNNNN.dat`<sup>[\[2\]](#note2)</sup> | Actual Gold BCR blocks (in network format, dumped in raw on disk, 128 MiB per file)
...
`./`               | `goldbcr.conf`        | Contains [configuration settings](goldbcr-conf.md) for `goldbcrd` or `goldbcr-qt`; can be specified by `-conf` option
`./`               | `goldbcrd.pid`        | Stores the process ID (PID) of `goldbcrd` or `goldbcr-qt` while running; created at start and deleted on shutdown; can be specified by `-pid` option
`./`               | `debug.log`           | Contains debug information and general logging generated by `goldbcrd` or `goldbcr-qt`; can be specified by `-debuglogfile` option
...
`goldbcr-qt` uses [`QSettings`](https://doc.qt.io/qt-5/qsettings.html) class; this implies platform-specific [locations where application settings are stored](https://doc.qt.io/qt-5/qsettings.html#locations-where-application-settings-are-stored).

## Legacy subdirectories and files

These subdirectories and files are no longer used by GBCR:
