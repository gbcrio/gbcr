...
- [Development guidelines](#development-guidelines)
    - [General Gold BCR Core](#general-goldbcr-core)
    - [Wallet](#wallet)
    - [General C++](#general-c)
...
Gold BCR Core uses [Doxygen](http://www.doxygen.nl/) to generate its official documentation.
...
Run with the `-testnet` option to run with "play goldbcrs" on the test network, if you
...
Gold BCR Core is a multi-threaded application, and deadlocks or other
...
```shell
$ valgrind --suppressions=contrib/valgrind.supp src/test/test_goldbcr
$ valgrind --suppressions=contrib/valgrind.supp --leak-check=full \
      --show-leak-kinds=all src/test/test_goldbcr --log_level=test_suite
$ valgrind -v --leak-check=full src/goldbcrd -printtoconsole
...
# A coverage report will now be accessible at `./test_goldbcr.coverage/index.html`.
```
...
To profile a running goldbcrd process for 60 seconds, you could use an
invocation of `perf record` like this:

```sh
$ perf record \
    -g --call-graph dwarf --per-thread -F 140 \
    -p `pgrep goldbcrd` -- sleep 60
```
...
Gold BCR Core can be compiled with various "sanitizers" enabled, which add
...
However, in open source software such as Gold BCR Core, where everyone uses
...
pay attention to for reviewers of Gold BCR Core code.

General Gold BCR Core
----------------------
...
- For `strprintf`, `LogPrint`, `LogPrintf` formatting characters don't need size specifiers.
  ...
  - *Rationale*: Gold BCR Core uses tinyformat, which is type safe. Leave them out to avoid confusion.
...
Some of these are maintained by active developers of Gold BCR Core, in which case changes should probably go
directly upstream without being PRed directly against the project. They will be merged back in the next
subtree merge.

Others are external projects without a tight relationship with our project. Changes to these should also
be sent upstream, but bugfixes may also be prudent to PR against Gold BCR Core so that they can be integrated
...
- src/secp256k1
  - Upstream at https://github.com/goldbcr-core/secp256k1/ ; actively maintained by Core contributors.

- src/crypto/ctaes
  - Upstream at https://github.com/goldbcr-core/ctaes ; actively maintained by Core contributors.

- src/univalue
  - Upstream at https://github.com/goldbcr-core/univalue ; actively maintained by Core contributors, deviates from upstream https://github.com/jgarzik/univalue
...
file descriptors, it will cause problems with Gold BCR's `select()` loop, because
...
compatibility between nodes. This happened in Gold BCR 0.8 (when LevelDB was
...
would be to revert the upstream fix before applying the updates to Gold BCR's
...
- Try not to overload methods on argument type. E.g. don't make `getblock(true)` and `getblock("hash")`
  ...
  - *Rationale*: This is impossible to use with `goldbcr-cli`, and can be surprising to users.
...
- Add every non-string RPC argument `(method, idx, name)` to the table `vRPCConvertParams` in `rpc/client.cpp`.
  ...
  - *Rationale*: `gold-bcr-cli` and the GUI debug console use this table to determine how to
...
- Wallet RPCs call BlockUntilSyncedToCurrentChain to maintain consistency with
  ...
  - *Rationale*: In previous versions of Gold BCR Core, the wallet was always
...
- For consistency and friendliness to code generation tools, interface method
  ...
  before in [#14635](https://github.com/goldbcr/goldbcr/pull/14635).
