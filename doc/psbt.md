# PSBT Howto for Gold BCR Core

Since Gold BCR Core 0.17, an RPC interface exists for Partially Signed Gold BCR
...
PSBT is an interchange format for Gold BCR transactions that are not fully signed
...
Overall, the construction of a fully signed Gold BCR transaction goes through the
following steps:
...
- An **Extractor** produces a valid Gold BCR transaction (in network format)
...
#### Multisig with multiple Gold BCR Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Gold BCR Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `goldbcr-cli` is used.
