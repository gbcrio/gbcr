...
from test_framework.test_framework import GoldBCRTestFramework, SkipTest
...
class BackwardsCompatibilityTest(GoldBCRTestFramework):
    ...
    def setup_nodes(self):
        ...
        self.add_nodes(self.num_nodes, extra_args=self.extra_args, versions=[
            ...
            self.options.goldbcrd,
            self.options.goldbcrd,
            releases_path + "/v0.19.0.1/bin/goldbcrd",
            releases_path + "/v0.18.1/bin/goldbcrd",
            releases_path + "/v0.17.1/bin/goldbcrd"
        ], binary_cli=[
            self.options.goldbcrcli,
            self.options.goldbcrcli,
            releases_path + "/v0.19.0.1/bin/goldbcr-cli",
            releases_path + "/v0.18.1/bin/goldbcr-cli",
            releases_path + "/v0.17.1/bin/goldbcr-cli"
        ])
    ...
    def run_test(self):
        ...
        # RPC loadwallet failure causes goldbcrd to exit, in addition to the RPC
        # call failure, so the following test won't work:
        # assert_raises_rpc_error(-4, "Wallet loading failed.", node_v17.loadwallet, 'w3_v18')

        # Instead, we stop node and try to launch it with the wallet:
        self.stop_node(self.num_nodes - 1)
        node_v17.assert_start_raises_init_error(["-wallet=w3_v18"], "Error: Error loading w3_v18: Wallet requires newer version of Gold BCR Core")
        node_v17.assert_start_raises_init_error(["-wallet=w3"], "Error: Error loading w3: Wallet requires newer version of Gold BCR Core")
        ...
        # Copy the 0.17 wallet to the last Gold BCR Core version and open it:
        ...
        # Copy the 0.19 wallet to the last Gold BCR Core version and open it:
        shutil.copytree(
